...
public class ActionHandler {
	...
	private void updateCatalogUiSettings() {
		...
		//filteredRecords = Integer.valueOf(catalogUi.filteredLabel.getText().toString());
	...
	private void updateCatalogTable(SimpleListener aListener) {
		...
		sortedFilteredList = sortedList.stream()
		...
		if (isMagLimitsSelected) {
			...
			sortedFilteredList = sortedFilteredList.stream()
											.filter(p -> (p.getMag() >= lowerLimit))
											.filter(p -> p.getMag() <= upperLimit)
											.collect(Collectors.toList());
		}
	...
	public static void main(String args[]) {
		...
		List<FieldObject> sortByDeltaMag = fieldObjects.stream()
												.sorted(Comparator.comparingDouble(p -> Math.abs((p.getMag()-tgtMag))))
												.collect(Collectors.toList());
		idx = 2;	
